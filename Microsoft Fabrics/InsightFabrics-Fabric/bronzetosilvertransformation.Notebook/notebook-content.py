...
unprocessed_df = spark.sql("SELECT batch_id FROM LhInsightFabric.bronze.bronze_batch_log WHERE silver_processed = false")
batch_list = [f"'{row.batch_id}'" for row in unprocessed_df.collect()]
...
if not batch_list:
    ...
    mssparkutils.notebook.exit("Success: No batches found") 
else:
    # Join the IDs into a string: "'ID1', 'ID2', 'ID3'"
    batch_ids_str = ", ".join(batch_list)

    # 2. Pull ALL data for these batches in one go
    # This is much faster for Spark than multiple small reads
    query=f""" 
        SELECT * FROM bronze.tbl_rawEmotiondata 
        WHERE Batch_RunId IN ({batch_ids_str})
    """
...
from pyspark.sql.functions import to_timestamp, regexp_replace,substring,current_timestamp
clean_df=clean_df.withColumn("SilverLoadTimestamp",current_timestamp())
...
df_Invalid.write.mode("append").format("delta").option("mergeSchema", "true").saveAsTable("LhInsightFabric.quarrentine.Invalid_emotion_events")
...
df_valid.write.mode("append").format("delta").option("mergeSchema", "true").saveAsTable("LhInsightFabric.silver.silver_emotion_events")
