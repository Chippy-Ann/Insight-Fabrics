...
this_month = spark.sql("SELECT date_format(current_timestamp(), 'yyyy-MM')").collect()[0][0]
last_month = spark.sql("SELECT date_format(add_months(current_timestamp(), -1), 'yyyy-MM')").collect()[0][0]
#get data of current month
df_silver = spark.read.table("LhInsightFabric.silver.silver_emotion_events") \
    .filter(F.date_format("SilverLoadTimestamp", "yyyy-MM") == this_month)

# Fallback to Last Month if current is empty
if df_silver.count() == 0:
    print(f"No data for {this_month}, checking {last_month}...")
    df_silver = spark.read.table("LhInsightFabric.silver.silver_emotion_events") \
        .filter(F.date_format("SilverLoadTimestamp", "yyyy-MM") == last_month)

#Final check
if df_silver.count() == 0:
    mssparkutils.notebook.exit("No data found for current or previous month.")

print(f"Processing {df_silver.count()} rows for month: {this_month if df_silver.count() > 0 else last_month}")
